...
class DeckTest {
    ...
    void testSubDeck() {
        ...
        assertEquals("[2 trefl, 3 trefl]", sub.toString());
        ...
        Throwable thrown = assertThrows(
                NegativeArraySizeException.class, () -> deck.subDeck(2,0)
        );
        // they are equivalent
        assertEquals("-2" ,thrown.getMessage());
        // assertTrue(thrown.getMessage().equals("-2"));
    ...
    void merge() {
        ...
        assertAll(
                () -> assertEquals("[2 trefl, as trefl, as karo]", (Deck.merge(d, d2).toString())),
                () -> assertEquals("[2 trefl, 2 trefl, as trefl, as trefl]", (Deck.merge(d2, d2).toString())),
                () -> assertEquals("[2 trefl, as trefl]", (Deck.merge(d0, d2).toString())),
                () -> assertEquals("", (Deck.merge(d0, d0).toString()))
        );
    }
